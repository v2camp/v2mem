...
故「`mem notes` 输出类码」有两种读法：

| 候选 | 含义 | 状态 |
|:---|:---|:---|
| **A** | `mem notes` 扩展为 L1 可注入内容的**统一生成器**：先输出类码表（常数 4 行），再输出硬规则小字条 | ✅ **已采纳**（2026-09-18，项目所有者裁定） |
| B | 新增独立命令（如 `mem codes`）专管类码表输出，`notes` 不动 | ❌ 未采纳 |

**采纳 A 的直接后果（WS1 任务边界据此确定，非可选项）：**

1. `renderNotes` 改为**两段输出**：类码表 + 硬规则小字条。两段都必须稳定幂等
   （沿用既有 `(project, content, id)` 排序纪律）。
2. `mem hook` 若需复用，**消费同一 `renderNotes`**，不得另起第二个来源 ——
   否则"两个来源、预算各算一遍"的重复记账问题就回来了（这正是选 A 要避免的）。
3. **预算必须一次算清**：类码表是常数 4 行、硬规则小字条是可变部分，
   上限约束的是**两者合计**。故 WS1.3 需要一条「合计预算」断言，而不只是各自不超限。
4. `--json` 模式下两段必须**结构化可分辨**（不得靠解析文本切分）。
...
> **已裁定为候选 A**（见上「前置裁决」）：直接改 `notes.go` 的 `renderNotes`，不新增子命令。

- [ ] **Step 1: 写失败测试**：断言输出**首段为类码表**（4 行，顺序 `P/D/X/T`）、
  **其后为硬规则小字条**；断言多次运行输出逐字节一致（沿用既有幂等纪律）；
  断言 `--json` 模式下两块内容**结构化可分辨**（不得靠解析文本切分）。
- [ ] **Step 2: 跑测试确认失败。**
- [ ] **Step 3: 实现** `renderNotes` 两段输出（类码表来自 WS1.1 的 `kindCodes` 常数，
  硬规则沿用既有 `collectRules`）。
- [ ] **Step 4: 加「合计预算」断言**（采纳 A 的后果 ③）：类码表 4 行 + 硬规则小字条的
  **合计** rune 数 ≤ 2048。造一批足够长的硬规则（≥10 条）把这条断言逼到真的会失败，
  否则等于没断言。
- [ ] **Step 5: 跑测试确认通过。**
- [ ] **Step 6: 回归**：`mem hook` 的注入体量与去重逻辑不受影响；
  确认 hook **没有**新起第二个类码来源（make cover-gate + 既有 hook 用例全绿）。
- [ ] **Step 7: Commit**（`feat: mem notes 输出类码表`）。
